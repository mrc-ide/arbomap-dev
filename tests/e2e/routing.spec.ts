...
const expectIndexPage = async (page, url: string, selectedIndicator: string, selectedCountry: string, scale: string,
                               featureCount: number, selectedCountryFeatureCount: number) => {
    ...
    await expect(await summary.getAttribute("selected-country-feature-count")).toBe(selectedCountryFeatureCount.toString());
};
...
test.describe("Router", () => {
    ...
    test("shows not found details when non-existent indicator and country", async ({page}) => {
        ...
        await expect( await page.getByText("Unknown indicator: notAnIndicator. Unknown country ISO: notAnISO.")).toBeVisible();
    });
});
